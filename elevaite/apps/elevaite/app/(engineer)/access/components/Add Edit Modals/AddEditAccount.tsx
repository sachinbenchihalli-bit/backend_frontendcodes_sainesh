import { CommonFormLabels, CommonInput, SimpleTextarea } from "@repo/ui/components";
...
export function AddEditAccount(props: AddEditAccountProps): JSX.Element {
    ...
    const [accountDescription, setAccountDescription] = useState(props.account?.description ?? "");


    async function handleClick(): Promise<void> {
        if (!accountName) return;
        let result: boolean;
        if (props.account) result = await rolesContext.editAccount(props.account.id, accountName, accountDescription);
        else result = await rolesContext.addAccount(accountName, accountDescription);
    ...
    return (
        <div className="add-edit-account-container">
            ...
            <AddEditBaseDialog
                ...
                disabled={!accountName || (props.account && props.account.name === accountName && props.account.description === accountDescription)}
