...
export function ChatContextProvider(
  ...
  async function getServerChatbotResponse(
    ...
    const userId = session.data?.user?.id||"Unknown user";
    // if (!userId) return;
  ...
  async function handleServerChatbotResponse(
    ...
    if (!messageText || !passedSession) return;
    setIsChatLoading(true);
    const MAX_PAYLOAD_HISTORY = 3;
    let conversation_payload: ChatBotPayload[] = [];

    for (let i = 0; i < passedSession.messages.length; i++) {
      if (i >= MAX_PAYLOAD_HISTORY) break;
      const message = passedSession.messages[passedSession.messages.length - i - 1];
      conversation_payload.push({
        actor: message.isBot ? "system" : "user",
        content: message.text
      });
    }

    conversation_payload = conversation_payload.reverse();
    // const userIdNumbersList = passedSession.messages
    // .filter((item) => !item.isBot) // Only include non-bot messages
    // .map((userItem) =>
    //   Number(userItem.id.slice(USER_MESSAGE_ID_PREFIX.length))
    // );
    // const newId =
    ...
    //   text: messageText,
    //   };
    // const newSession = updateSessionListWithNewMessage(newMessage, passedSession);
    console.log("Hi");
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL ?? ""}/`, {
        ...
        headers: {
          ...
          "X-Token": "coneofsilence",  // Example auth token, replace with your actual token if necessary
        },
        body: JSON.stringify({
          conversation_payload: conversation_payload,
          query: messageText, // The user query
          skip_llm_call: false, // Depending on your needs
          use_openai_directly: true, // Use OpenAI directly or your custom logic
        }),
      });
      // console.log("before?");
