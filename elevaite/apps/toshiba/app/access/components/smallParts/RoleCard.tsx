...
export function RoleCard(props: RoleCardProps): JSX.Element {
  const test = "Test";
  const secondaryTest =
    "This is a very long sentence just to see what's going on and if it's behaving as it should.";
  ...
  return (
    <CommonButton
      className={[
        "role-card-container",
        props.role.id === props.selectedRoleId ? "selected" : undefined,
      ]
        .filter(Boolean)
        .join(" ")}
      onClick={handleClick}
    ...
    >
      <div className="role-card-line">
        ...
        {!props.role.name ? undefined : ( // Fix this
          <>
            <span>|</span>
            <span className="secondary cut-overflow" title={test}>
              {test}
            </span>
          </>
        )}
      ...
      <div className="role-card-line">
        ...
        {!props.role.name ? undefined : ( // Fix this
          <>
            <span>•</span>
            <span className="cut-overflow" title={secondaryTest}>
              {secondaryTest}
            </span>
          </>
        )}
