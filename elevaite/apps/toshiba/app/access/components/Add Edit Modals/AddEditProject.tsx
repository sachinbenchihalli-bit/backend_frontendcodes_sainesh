...
export function AddEditProject(props: AddEditProjectProps): JSX.Element {
  ...
  useEffect(() => {
    setSelectedParentProjectId(
      selectedAccountId === props.project?.account_id &&
        props.project?.parent_project_id
        ? props.project.parent_project_id
        : ""
    );
  ...
  async function handleClick(): Promise<void> {
    ...
    let result: boolean;
    if (props.project)
      result = await rolesContext.editProject(
        props.project.id,
        projectName,
        projectDescription
      );
    else
      result = await rolesContext.addProject(
        selectedAccountId,
        projectName,
        projectDescription,
    ...
    <div className="add-edit-project-container">
      <AddEditBaseDialog
        ...
        disabled={
          !projectName ||
          (props.project &&
            props.project.name === projectName &&
            props.project.description === projectDescription)
        }
      ...
      >
        ...
        <CommonFormLabels
          ...
          info={
            props.project
              ? "Parent Project cannot be edited"
              : "Select an account to see available parent projects"
          }
        >
          <CommonSelect
        ...
        <CommonFormLabels label="Project Description">
          <SimpleTextarea
