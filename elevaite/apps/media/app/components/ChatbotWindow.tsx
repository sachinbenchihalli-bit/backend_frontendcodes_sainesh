...
export function ChatbotWindow({onQueryClick, onGenerateIOClick}): JSX.Element {
            ...
            <div className="chatbot-window-scroller">
                <div className="chatbot-window-message-list" ref={messageListRef}>
                    {chatContext.selectedSession?.messages.map(message =>
                        <ChatMessage key={message.id} {...message} onQueryClick={onQueryClick} onGenerateIOClick={onGenerateIOClick}/>
                    )}
                </div>
            </div>
